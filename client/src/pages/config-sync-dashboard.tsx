...
export default function ConfigSyncDashboard() {
  ...
  const mockSyncStatus: ConfigSyncStatus[] = [
    {
      ...
      syncStatus: "success",
      configModules: {
        auth: { status: "synced", lastUpdated: new Date(Date.now() - 300000).toISOString(), version: "1.2.0" },
        rbac: { status: "synced", lastUpdated: new Date(Date.now() - 300000).toISOString(), version: "1.1.0" },
        notifications: { status: "synced", lastUpdated: new Date(Date.now() - 300000).toISOString(), version: "1.0.5" },
        modules: { status: "synced", lastUpdated: new Date(Date.now() - 300000).toISOString(), version: "2.0.0" }
      },
      conflicts: 0,
      pendingChanges: 0
    },
    {
      ...
      syncStatus: "partial",
      configModules: {
        auth: { status: "synced", lastUpdated: new Date(Date.now() - 900000).toISOString(), version: "1.2.0" },
        rbac: { status: "conflict", lastUpdated: new Date(Date.now() - 1800000).toISOString(), version: "1.0.8" },
        notifications: { status: "synced", lastUpdated: new Date(Date.now() - 900000).toISOString(), version: "1.0.5" },
        modules: { status: "pending", lastUpdated: new Date(Date.now() - 1800000).toISOString(), version: "1.9.0" }
      },
      conflicts: 1,
      pendingChanges: 2
    },
    {
      ...
      syncStatus: "failed",
      configModules: {
        auth: { status: "error", lastUpdated: new Date(Date.now() - 3600000).toISOString(), version: "1.1.5" },
        rbac: { status: "outdated", lastUpdated: new Date(Date.now() - 7200000).toISOString(), version: "1.0.3" },
        notifications: { status: "outdated", lastUpdated: new Date(Date.now() - 7200000).toISOString(), version: "1.0.2" },
        modules: { status: "error", lastUpdated: new Date(Date.now() - 3600000).toISOString(), version: "1.8.0" }
      },
      conflicts: 3,
      pendingChanges: 5
    }
  ...
  const healthPercentage = Math.round((overallStats.healthyTenants / overallStats.totalTenants) * 100);
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              of {connectionStats?.total || 0} total
            </p>
          </CardContent>
        </Card>
      </div>
                  ...
                  <div key={tenant.tenantId} className="border rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-4 gap-4 mb-3">
                      {Object.entries(tenant.configModules).map(([module, config]) => (
                        <div key={module} className="flex items-center gap-2 p-2 bg-slate-50 rounded">
                          ...
                          <div>
                            ...
                            <div className="space-y-1 text-sm">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="font-medium text-blue-600">{tenant.pendingChanges}</span>
                              </div>
                              <div className="flex justify-between">
                                <span>Last Sync:</span>
                                <span>{format(new Date(tenant.lastSyncAt), 'MMM d, yyyy HH:mm:ss')}</span>
                              </div>
                            </div>
                          </div>
                        </div>
              ...
              <div className="space-y-3">
                {syncEvents.length > 0 ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm">Events will appear here as configurations are synchronized</p>
                  </div>
                )}
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">Conflicts will appear here when platform and tenant configurations diverge</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
