...
import { pgTable, text, varchar, timestamp, boolean, integer, uuid, jsonb, decimal, bigint } from "drizzle-orm/pg-core";
...
export const tenantNotifications = pgTable("tenant_notifications", {
  ...
  channels: text("channels").array().notNull().default(sql`'{email}'::text[]`), // email, sms, push, webhook, in_app
...
export const userNotificationPreferences = pgTable("user_notification_preferences", {
  ...
  channels: jsonb("channels").notNull().default(sql`'{"email": true, "sms": false, "push": false, "webhook": false, "in_app": true}'`),
  quietHours: jsonb("quiet_hours").default(sql`'{"enabled": false, "start": "22:00", "end": "08:00", "timezone": "UTC"}'`),
...
export const notificationDeliveries = pgTable("notification_deliveries", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  notificationId: uuid("notification_id").references(() => tenantNotifications.id, { onDelete: "cascade" }).notNull(),
...
export const backupJobs = pgTable("backup_jobs", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  configurationId: uuid("configuration_id").references(() => backupConfigurations.id, { onDelete: "cascade" }).notNull(),
...
export const restoreJobs = pgTable("restore_jobs", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  backupJobId: uuid("backup_job_id").references(() => backupJobs.id).notNull(),
...
export const serviceHealthChecks = pgTable("service_health_checks", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  serviceId: uuid("service_id").references(() => infrastructureServices.id, { onDelete: "cascade" }).notNull(),
...
export const drTestExecutions = pgTable("dr_test_executions", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  planId: uuid("plan_id").references(() => disasterRecoveryPlans.id, { onDelete: "cascade" }).notNull(),
...
export const maintenanceWindows = pgTable("maintenance_windows", {
  ...
  scheduledBy: uuid("scheduled_by").references(() => users.id).notNull(),
  approvedBy: uuid("approved_by").references(() => users.id),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow()
});
...
export const samlSessions = pgTable("saml_sessions", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  samlConfigId: uuid("saml_config_id").references(() => samlConfigs.id, { onDelete: "cascade" }).notNull(),
  tenantId: uuid("tenant_id").references(() => tenants.id, { onDelete: "cascade" }).notNull(),
...
export const roleHierarchy = pgTable("role_hierarchy", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").references(() => tenants.id, { onDelete: "cascade" }).notNull(),
  parentRoleId: uuid("parent_role_id").references(() => tenantRoles.id, { onDelete: "cascade" }).notNull(),
  childRoleId: uuid("child_role_id").references(() => tenantRoles.id, { onDelete: "cascade" }).notNull(),
...
export const bulkOperations = pgTable("bulk_operations", {
  ...
  initiatedBy: uuid("initiated_by").references(() => tenantUsers.id).notNull(),
  startedAt: timestamp("started_at"),
  completedAt: timestamp("completed_at"),
  createdAt: timestamp("created_at").defaultNow()
});
...
export const permissionAuditLog = pgTable("permission_audit_log", {
  ...
  changedBy: uuid("changed_by").references(() => tenantUsers.id).notNull(),
