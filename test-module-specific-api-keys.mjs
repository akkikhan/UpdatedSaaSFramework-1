...
/**
 * Test async function checkServerHealth() {
  try {
...
}fic API Key Generation During Tenant Onboarding
 *
...
async function testModuleSpecificApiKeys() {
  ...
  const testCases = [
    ...
    {
      name: "Authentication + Notifications Only",
      enabledModules: ["authentication", "notifications"],
      expectedApiKeys: ["authApiKey", "notificationsApiKey"],
      shouldNotHave: ["rbacApiKey", "loggingApiKey"],
    },
  ];
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    const tenantData = {
      ...
      authProvider: "local",
      sendEmail: false, // Disable email for faster testing
    };
...
async function testEmailContentGeneration() {
  ...
  const tenantData = {
    ...
    sendEmail: true, // Enable email to test content generation
  };
  ...
  try {
    ...
    colorLog("green", `   ✅ Logging API Key: ${tenant.loggingApiKey.substring(0, 8)}...`);
    colorLog(
      "green",
...
async function testProviderConfiguration() {
  colorLog("blue", "\n⚙️ Testing Provider-Specific Configuration");

  const providers = ["azure-ad", "auth0", "local", "saml"];

  for (const provider of providers) {
    const orgId = `test-provider-${provider}-${Date.now()}`;
    const tenantData = {
      name: `${provider.toUpperCase()} Test Tenant`,
      orgId: orgId,
      adminEmail: `admin-${orgId}@test.com`,
      enabledModules: ["authentication", "rbac"],
    ...
    try {
      colorLog("yellow", `   Testing ${provider} provider...`);
      ...
      colorLog("green", `   ✅ ${provider} provider tenant created: ${tenant.orgId}`);
      colorLog("green", `   ✅ Auth Provider: ${tenant.authProvider}`);
    } catch (error) {
      colorLog("red", `   ❌ Provider ${provider} test failed: ${error.message}`);
      return false;
    }
  }
...
async function runAllTests() {
  ...
  const tests = [
    ...
    { name: "Provider-Specific Configuration", fn: testProviderConfiguration },
  ];
  ...
  if (allPassed) {
    ...
    colorLog("green", "✅ Provider-specific configurations are supported");
