...
import { insertTenantSchema, insertUserSchema, insertTenantUserSchema, insertTenantRoleSchema, insertTenantUserRoleSchema, insertPermissionTemplateSchema, insertBusinessTypeSchema, insertDefaultRoleSchema, deviceTokens } from "@shared/schema";
import { db } from "./db";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/v2/auth/login", async (req, res) => {
    try {
      ...
      if (!result.success) {
        const statusCode = result.requiresMFA ? 202 : 
                          result.lockedUntil ? 423 : 401;
  ...
  app.patch("/api/tenants/:tenantId/roles/:roleId", async (req, res) => {
    try {
      ...
      if (name && name !== existingRole.name) {
        const roleExists = existingRoles.some(role => 
          role.id !== roleId && role.name.toLowerCase() === name.toLowerCase()
        );
      ...
      await storage.logSystemActivity({
        ...
        entityType: 'tenant_role',
        entityId: roleId,
        details: { name: name || existingRole.name, permissions: permissions || existingRole.permissions },
        ipAddress: req.ip,
        userAgent: req.get('User-Agent')
      });
  ...
  app.delete("/api/tenants/:tenantId/roles/:roleId", async (req, res) => {
    try {
      ...
      const usersWithRole = tenantUsers.filter(user => 
        (user as any).roles && (user as any).roles.some((role: any) => role.id === roleId)
      );
  ...
  app.patch("/api/tenants/:tenantId/rbac-config", async (req, res) => {
    try {
      ...
      await storage.updateTenantModules(tenantId, tenant.enabledModules as string[], updatedModuleConfigs);
      ...
      if (businessType && businessType !== currentModuleConfigs.rbac?.businessType) {
        ...
        const selectedBusinessType = businessTypes.find(bt => 
          bt.name.toLowerCase().replace(/\s+/g, '-') === businessType
        );
  ...
  app.post("/api/rbac-config/seed-sample-data", async (req, res) => {
    try {
      ...
      const standardTemplate = await storage.createPermissionTemplate({
        ...
        permissions: ['read_users', 'create_users', 'update_users', 'manage_settings', 'read_reports'],
      ...
      const healthcareTemplate = await storage.createPermissionTemplate({
        ...
        permissions: ['read_users', 'patient_data_access', 'hipaa_audit_access', 'compliance_reports', 'security_logs'],
      ...
      const bankingTemplate = await storage.createPermissionTemplate({
        ...
        permissions: ['read_users', 'create_users', 'financial_data_access', 'transaction_processing', 'account_management', 'wire_transfers', 'loan_processing', 'credit_analysis', 'aml_monitoring', 'kyc_verification', 'regulatory_reporting', 'sox_compliance_access', 'basel_reporting', 'risk_assessment', 'fraud_detection', 'audit_trail_access'],
      ...
      const insuranceTemplate = await storage.createPermissionTemplate({
        ...
        permissions: ['read_users', 'create_users', 'policy_management', 'claims_processing', 'underwriting_access', 'actuarial_data_access', 'premium_calculation', 'risk_modeling', 'insurance_compliance_reports', 'solvency_reporting', 'reinsurance_management', 'regulatory_filings'],
      ...
      const bankingBusiness = await storage.createBusinessType({
        ...
        defaultPermissions: ['read_users', 'financial_data_access', 'transaction_processing', 'account_management', 'aml_monitoring', 'kyc_verification', 'regulatory_reporting'],
        riskLevel: 'critical',
        isActive: true,
        maxTenants: 25
      });
      ...
      const insuranceBusiness = await storage.createBusinessType({
        ...
        defaultPermissions: ['read_users', 'policy_management', 'claims_processing', 'underwriting_access', 'actuarial_data_access', 'regulatory_filings'],
        riskLevel: 'high',
        isActive: true,
        maxTenants: 30
      });
      ...
      await storage.createDefaultRole({
        ...
        permissions: ['read_users', 'create_users', 'update_users', 'delete_users', 'manage_roles', 'manage_permissions', 'admin_panel_access'],
      ...
      await storage.createDefaultRole({
        ...
        permissions: ['read_users', 'create_users', 'patient_data_access', 'hipaa_audit_access', 'compliance_reports'],
      ...
      await storage.createDefaultRole({
        ...
        permissions: ['read_users', 'create_users', 'update_users', 'financial_data_access', 'transaction_processing', 'account_management', 'wire_transfers', 'regulatory_reporting', 'audit_trail_access', 'manage_roles'],
      ...
      await storage.createDefaultRole({
        ...
        permissions: ['read_users', 'aml_monitoring', 'kyc_verification', 'regulatory_reporting', 'sox_compliance_access', 'basel_reporting', 'audit_trail_access', 'risk_assessment'],
        businessTypeId: bankingBusiness.id,
        permissionTemplateId: bankingTemplate.id,
        isSystemRole: false,
        canBeModified: true,
      ...
      await storage.createDefaultRole({
        ...
        permissions: ['read_users', 'account_management', 'transaction_processing', 'kyc_verification'],
        businessTypeId: bankingBusiness.id,
        permissionTemplateId: bankingTemplate.id,
        isSystemRole: false,
        canBeModified: true,
      ...
      await storage.createDefaultRole({
        ...
        permissions: ['read_users', 'account_management', 'loan_processing', 'credit_analysis', 'kyc_verification', 'risk_assessment'],
        businessTypeId: bankingBusiness.id,
        permissionTemplateId: bankingTemplate.id,
        isSystemRole: false,
        canBeModified: true,
      ...
      await storage.createDefaultRole({
        ...
        permissions: ['read_users', 'create_users', 'update_users', 'policy_management', 'claims_processing', 'underwriting_access', 'actuarial_data_access', 'regulatory_filings', 'manage_roles'],
      ...
      await storage.createDefaultRole({
        ...
        permissions: ['read_users', 'policy_management', 'underwriting_access', 'actuarial_data_access', 'risk_modeling', 'premium_calculation'],
        businessTypeId: insuranceBusiness.id,
        permissionTemplateId: insuranceTemplate.id,
        isSystemRole: false,
        canBeModified: true,
      ...
      await storage.createDefaultRole({
        ...
        permissions: ['read_users', 'actuarial_data_access', 'risk_modeling', 'premium_calculation', 'solvency_reporting', 'catastrophe_modeling'],
        businessTypeId: insuranceBusiness.id,
        permissionTemplateId: insuranceTemplate.id,
        isSystemRole: false,
        canBeModified: true,
      ...
      res.json({ 
        ...
        data: {
          permissionTemplates: [standardTemplate, healthcareTemplate, bankingTemplate, insuranceTemplate],
          businessTypes: [generalBusiness, healthcareBusiness, financeBusiness, bankingBusiness, insuranceBusiness],
          defaultRoles: [
            'Administrator', 'Manager', 'Healthcare Admin',
            'Bank Administrator', 'Compliance Officer', 'Bank Teller', 'Loan Officer',
            'Insurance Administrator', 'Underwriter', 'Claims Adjuster', 'Actuary'
          ]
        }
      });
    } catch (error) {
  ...
  app.delete("/api/v2/user/device-tokens/:token", authMiddleware, async (req, res) => {
    try {
      ...
      await db.update(deviceTokens)
        .set({ isActive: false })
        .where(and(
          eq(deviceTokens.token, req.params.token),
  ...
  app.post("/api/v2/admin/notification-configs/sms", authMiddleware, async (req, res) => {
    try {
      ...
      const [created] = await db.insert(smsConfigs).values({
  ...
  app.post("/api/v2/admin/notification-configs/push", authMiddleware, async (req, res) => {
    try {
      ...
      const [created] = await db.insert(pushConfigs).values({
        tenantId: user.tenantId,
        provider,
        platform,
        config,
  ...
  app.post("/api/v2/admin/infrastructure-services", authMiddleware, async (req, res) => {
    try {
      const { name, serviceType, endpoint, checkType, checkInterval, timeout, retryCount, expectedResponse, alertThreshold, isCritical, dependencies, tags } = req.body;
  ...
  app.get("/api/v2/admin/service-health", authMiddleware, async (req, res) => {
    try {
      ...
      .leftJoin(infrastructureServices, eq(serviceHealthChecks.serviceId, infrastructureServices.id))
  ...
  app.get("/api/v2/admin/infrastructure-dashboard", authMiddleware, async (req, res) => {
    try {
      ...
      const servicesSummary = await db.select({
      ...
      const criticalServices = servicesSummary.filter(s => s.isCritical && s.latestStatus !== 'healthy').length;
  ...
  app.post("/api/v2/admin/dr-plans", authMiddleware, async (req, res) => {
    try {
      const user = (req as any).user;
      const { name, description, planType, rto, rpo, priority, triggerConditions, procedures, requiredResources, testSchedule } = req.body;
  ...
  app.post("/api/v2/admin/dr-plans/:planId/test", authMiddleware, async (req, res) => {
    try {
      ...
      const testId = await backupInfrastructureService.executeDRTest(
        planId, 
        testType, 
        user.userId
      );
  ...
  app.post("/api/v2/admin/maintenance-windows", authMiddleware, async (req, res) => {
    try {
      const user = (req as any).user;
      const { title, description, maintenanceType, impact, affectedServices, startTime, endTime } = req.body;
      
      const { maintenanceWindows } = await import("@shared/schema");
      
      const [created] = await db.insert(maintenanceWindows).values({
        title,
        description,
        maintenanceType,
        impact,
