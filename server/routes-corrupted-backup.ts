...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/platform/admins", platformAdminMiddleware, async (req, res) => {
    try {
      // Only super admins can list all platform admins
      ...
      res.status(500).json({ message: "Failed to fetch platform admins" });
    }
  });

  // Create new platform admin (super admin only)
  app.post("/api/platform/admins", platformAdminMiddleware, async (req, res) => {
    try {
      const { email, password, name, role = "admin" } = req.body;

      // Only super admins can create new platform admins
      if (req.platformAdmin!.role !== "super_admin") {
        return res.status(403).json({ message: "Super admin access required" });
      }
      ...
      if (existingAdmin) {
        return res.status(400).json({ message: "Platform admin with this email already exists" });
      }
      ...
      await storage.logSystemActivity({
        ...
        details: {
          ...
          name: admin.name,
          role: admin.role,
          createdBy: req.platformAdmin!.email,
        },
        ipAddress: req.ip,
      ...
      res.status(500).json({ message: "Failed to create platform admin" });
    }
  });
  ...
  // Tenant admin module configuration (TENANT ADMIN ONLY - can configure enabled modules, not enable/disable)
  app.patch(
    "/api/tenants/:id/module-config",
    authMiddleware,
    tenantMiddleware,
    async (req, res) => {
      try {
        ...
        // Verify user has permission to configure this tenant
        ...
        // Only allow configuration of already enabled modules
        const enabledModules = (tenant.enabledModules as string[]) || ["auth", "rbac"];
        const configKeys = Object.keys(moduleConfigs || {});

        // Check if trying to configure modules that aren't enabled
        const invalidConfigs = configKeys.filter(module => !enabledModules.includes(module));
        if (invalidConfigs.length > 0) {
          return res.status(400).json({
            message: "Cannot configure disabled modules",
            invalidModules: invalidConfigs,
            enabledModules,
          });
        }

        // Update only module configurations (not enabled modules list)
        ...
        await storage.logSystemActivity({
          ...
          entityType: "tenant",
          entityId: id,
          details: { updatedConfigs: configKeys, moduleConfigs },
          ipAddress: req.ip,
          userAgent: req.get("User-Agent"),
        });
        ...
        res.json({
          ...
          updatedModules: configKeys,
        });
      } catch (error) {
        console.error("Error updating tenant module configuration:", error);
        res.status(500).json({ message: "Failed to update module configuration" });
      }
    }
  );

  // Azure AD Configuration Management Routes (Platform Admin Only)

  // Configure Azure AD for a tenant
  app.post("/api/tenants/:id/azure-ad/config", platformAdminMiddleware, async (req, res) => {
    try {
      ...
      // Validate Azure AD configuration by testing it
      try {
        const testService = new AzureADService({
          tenantId,
          clientId,
          clientSecret,
        ...
        await testService.getAuthorizationUrl(["User.Read"], id);
      } catch (configError) {
        return res.status(400).json({
          message: "Invalid Azure AD configuration",
          details: configError instanceof Error ? configError.message : "Configuration test failed",
        });
      }
      ...
      moduleConfigs.auth.providers.push({
        ...
        config: {
          ...
          callbackUrl:
            callbackUrl ||
            `${process.env.BASE_URL || "http://localhost:3000"}/api/auth/azure/callback`,
        },
        userMapping: {
          emailField: "mail",
      ...
      await storage.logSystemActivity({
        ...
        details: {
          ...
          clientId,
          callbackUrl,
          configuredBy: req.platformAdmin!.email,
        },
        ipAddress: req.ip,
        userAgent: req.get("User-Agent"),
      });
  ...
  app.get("/api/tenants/:id/azure-ad/config", platformAdminMiddleware, async (req, res) => {
    try {
      const { id } = req.params;
      ...
      res.json({
        configured: true,
        provider: {
          type: azureProvider.type,
          name: azureProvider.name,
          enabled: azureProvider.enabled,
          config: {
            tenantId: azureProvider.config.tenantId,
            clientId: azureProvider.config.clientId,
            callbackUrl: azureProvider.config.callbackUrl,
            // Don't return clientSecret for security
          },
          userMapping: azureProvider.userMapping,
        },
      });
    } catch (error) {
      console.error("Error fetching Azure AD configuration:", error);
      res.status(500).json({ message: "Failed to fetch Azure AD configuration" });
    }
  });

  // Test Azure AD configuration
  app.post("/api/tenants/:id/azure-ad/test", platformAdminMiddleware, async (req, res) => {
    try {
      const { id } = req.params;
      ...
      const azureService = new AzureADService({
        ...
        redirectUri: azureProvider.config.callbackUrl,
      });

      // Generate a test auth URL to validate configuration
      const testAuthUrl = await azureService.getAuthorizationUrl(["User.Read"], id);

      // Log the test
      await storage.logSystemActivity({
        tenantId: id,
        action: "azure_ad_test",
        entityType: "tenant",
        entityId: id,
        details: {
          testResult: "success",
          testedBy: req.platformAdmin!.email,
        },
        ipAddress: req.ip,
        userAgent: req.get("User-Agent"),
      });
      ...
      res.json({
        ...
        testAuthUrl, // For debugging/validation purposes
      });
    } catch (error) {
      console.error("Error testing Azure AD configuration:", error);
      ...
      res.status(500).json({
        success: false,
        message: "Azure AD configuration test failed",
        error: error instanceof Error ? error.message : "Unknown error",
      });
    }
  });

  // Remove Azure AD configuration
  app.delete("/api/tenants/:id/azure-ad/config", platformAdminMiddleware, async (req, res) => {
    try {
      const { id } = req.params;
      ...
      await storage.logSystemActivity({
        tenantId: id,
        action: "azure_ad_removed",
        entityType: "tenant",
        entityId: id,
        details: {
          removedBy: req.platformAdmin!.email,
        },
        ipAddress: req.ip,
        userAgent: req.get("User-Agent"),
      });

      res.json({ message: "Azure AD configuration removed successfully" });
    } catch (error) {
      console.error("Error removing Azure AD configuration:", error);
      res.status(500).json({ message: "Failed to remove Azure AD configuration" });
    }
  });
  ...
  app.post("/api/platform/auth/refresh", async (req, res) => {
    try {
      ...
      const newToken = await platformAdminAuthService.refreshToken(refreshToken);

      if (!newToken) {
        return res.status(401).json({ message: "Invalid platform admin refresh token" });
      }

      res.json({ token: newToken });
    } catch (error) {
      console.error("Platform admin token refresh error:", error);
      res.status(500).json({ message: "Platform admin token refresh failed" });
    }
  });
  ...
  app.get("/api/auth/azure/callback", async (req, res) => {
    console.log("CALLBACK ROUTE EXECUTING - START"); // Direct test
    try {
      const { code, state, error } = req.query;

      console.log("CALLBACK ROUTE EXECUTING - INSIDE TRY"); // Direct test
      console.log("🔍 Azure AD Callback Debug:");
  ...
  app.post("/api/auth/azure/refresh", authMiddleware, async (req, res) => {
    try {
      const { account } = req.body; // MSAL account object instead of refresh token

      if (!account) {
        return res.status(400).json({ message: "Account information required for token refresh" });
      }
      ...
      // Refresh tokens using MSAL silent token acquisition
      const tokenResult = await azureADService.refreshTokens(account);
      ...
      console.error("Azure AD token refresh error:", error);
